...
#include "mt10t8.h"
    ...
    const uint8_t       char_array1[MT10T8_DIMENTION] = "0123456789";
    const uint8_t       char_array2[MT10T8_DIMENTION] = "AbcdEFJGhL";
    const uint8_t       char_array3[2 * MT10T8_DIMENTION] = "u-*_!^..A..\0";
    uint8_t             char_array4[2 * MT10T8_DIMENTION];
    
// Inicialization pt_A0, pt_D0, pt_D1, pt_D2, pt_D3, pt_WR1:
    const mt10t8_bus_t	mt10t8_bus = {PERIPH_BB_BASE + (((uint32_t)&(GPIOA->ODR)) & 0x000FFFFF) * 32 + GPIO_ODR_OD4_Pos * 4, \
									  ...
									  PERIPH_BB_BASE + (((uint32_t)&(GPIOA->ODR)) & 0x000FFFFF) * 32 + GPIO_ODR_OD5_Pos * 4};

// LCI description structure (pt_bus, pt_WR2):
    const mt10t8_t		mt10t8_1 = {&mt10t8_bus, PERIPH_BB_BASE + (((uint32_t)&(GPIOA->ODR)) & 0x000FFFFF) * 32 + GPIO_ODR_OD6_Pos * 4};

    mt10t8_init(&mt10t8_1);                                                                                         // Result: unlock and clear LCI.
	Delay(1000);
